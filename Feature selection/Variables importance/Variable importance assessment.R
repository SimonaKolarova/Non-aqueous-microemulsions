...
run_random_forest <- function(training_testing_data,predictors, model_performance, model_name, model_number) {
  ...
  model_performance <- model_performance_assessment(training_data, testing_data, predictors, model_performance, rf_model, model_name, model_number)
 
  # Variable importance
  varimp_dataframe <- as.data.frame(t(as.matrix(varImp(rf_model)$importance)))[1,]
  for (i in colnames(varimp_dataframe)) {
    for (j in colnames(model_performance)) {
      if (i == j) {
        model_performance[model_number, i] <- varimp_dataframe[j]
      }
    }
  }
